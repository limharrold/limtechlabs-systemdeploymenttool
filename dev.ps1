...
# Generates a unique timestamp to force the latest update from the server
...
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    # Appending the unique ID to bypass Cloudflare/ISP caching
...
try {
    Write-Host " Connecting to Global Server... " -NoNewline -ForegroundColor Gray
    ...
    Write-Host " [ ERROR ] Connection to server failed." -ForegroundColor Red
